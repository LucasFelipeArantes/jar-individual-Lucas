...
public class Memoria {
    public String memoria(String email) {
        ...
        if (porcentage <= 50) {
            ...
            fkAlerta = 4;
            logs.gravar("\nALERTA BAIXO - Memoria Utilizada %s%% NA CONTA: %s".formatted(porcentage.toString(), email));

            botSlack.notificarUsoMemoriaPorcentagem("Memoria" ,porcentage);

        } else if (porcentage > 75 && porcentage <= 90) {
            fkAlerta = 5;
            logs.gravar("\nALERTA MEDIO - Memoria Utilizada %s%% NA CONTA: %s".formatted(porcentage.toString(), email));

            botSlack.notificarUsoMemoriaPorcentagem("Memoria" ,porcentage);
        } else {
            fkAlerta = 6;
            logs.gravar("\nALERTA GRAVE - Memoria Utilizada %s%% NA CONTA: %s".formatted(porcentage, email));
