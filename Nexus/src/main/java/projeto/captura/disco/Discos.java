...
public class Discos {
    ...
    public String disco(String email) {
        ...
        if (porcentage <= 50) {
            ...
            fkAlerta = 7;
            logs.gravar("ALERTA BAIXO - Disco Utilizado %.2f%% NA CONTA: %s".formatted(porcentage, email));

            botSlack.notificarUsoDiscoPorcentagem("Disco", porcentage);
        } else if (porcentage > 75 && porcentage <= 90) {
            fkAlerta = 8;
            logs.gravar("ALERTA MEDIO - Disco Utilizado %.2f%% NA CONTA: %s".formatted(porcentage, email));

            botSlack.notificarUsoDiscoPorcentagem("Disco", porcentage);
        } else {
            fkAlerta = 9;
            logs.gravar("ALERTA GRAVE - Disco Utilizado %.2f%% NA CONTA: %s".formatted(porcentage, email));
