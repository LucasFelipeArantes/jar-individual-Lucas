...
public class Processador {
    public String processador(String email) {
        ...
        if (porcentage <= 50) {
            ...
            fkAlerta = 1;
            logs.gravar("\nALERTA BAIXO - CPU Utilizado %.2f%% NA CONTA: %s".formatted(porcentage, email));

            botSlack.notificarUsoCPUPorcentagem("CPU", porcentage);
        } else if (porcentage > 75 && porcentage <= 90) {
            fkAlerta = 2;
            logs.gravar("\nALERTA MEDIO - CPU Utilizado %.2f%% NA CONTA: %s".formatted(porcentage, email));

            botSlack.notificarUsoCPUPorcentagem("CPU", porcentage);
        } else {
            fkAlerta = 3;
            logs.gravar("\nALERTA GRAVE - CPU Utilizado %.2f%% NA CONTA: %s".formatted(porcentage, email));
